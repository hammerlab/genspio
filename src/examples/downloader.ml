...
let downloader () =
  ...
  let say strings =
    ...
    | s :: more ->
        seq
          ...
          @ [sayone (string "\n")] ) in
  let sayf fmt = Fmt.kstr (fun s -> say [string s]) fmt in
  let fail l = seq [say (string "ERROR: " :: l); fail "fail-list"] in
  let failf fmt = Fmt.kstr (fun s -> fail [string s]) fmt in
  ...
  let module Unwrapper = struct
    ...
    let remove_suffix v suf =
      no_newline_sed ~input:v (Fmt.str "s:^\\(.*\\)%s$:\\1:" suf)

    let to_switch name_variable t_list =
      let make_case t =
        case
          (string_matches_any name_variable#get [Fmt.str "\\.%s$" t.extension])
          [ say [Fmt.kstr string "%s: " t.verb; name_variable#get]
