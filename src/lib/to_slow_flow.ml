...
module Make_compiler (Parameters : sig
  val tmp_file_db : (string * string) list ref
end) =
struct
  (*md
 ...
 *)
    let pp fmt script =
      ...
      in
      fprintf fmt "%a\n# Result: %a\n" pp_command_list script.commands
        pp_result script.result
    ...
    let mktmp ~tmpdir ?expression ?script tag =
      if !direct_file_paths then
        ...
        Parameters.(tmp_file_db := (v, dir) :: !tmp_file_db) ;
        make []
          (*
...
*)
  let rec to_ir : type a. fail_commands:_ -> tmpdir:_ -> a t -> Script.t =
   fun ~fail_commands ~tmpdir e ->
    let continue : type a. a t -> _ =
     fun x -> to_ir ~fail_commands ~tmpdir x
    in
        ...
        let extra_check =
          ...
          | Tmp_file_in_variable v ->
              [ If_then_else
                  { condition=
                      sprintf
                        "od -t o1 -An -v ${%s} | grep ' 000' > /dev/null 2>&1 "
                        v
                  ; block_then=
                      ksprintf fail_commands
    ...
    | Redirect_output (unit_t, redirections) ->
        ...
        Script.sub_shell ~pre:pre_commands
          (sub_shell_commands @ uscript.commands)
    | Write_output {expr; stdout; stderr; return_value} ->
        ...
        let redirections =
          let make fd =
            Option.map ~f:(fun p ->
                {take= Construct.int fd; redirect_to= `Path p} )
          in
    ...
    | String_to_bool s ->
        ...
        let extra_check =
          ...
          If
            ...
            , No_op
            , Fail
                (sprintf "String-to-Bool: %S" (Script.to_argument scr.result))
            )
        in
    ...
    | Int_bin_comparison (ia, op, ib) ->
        ...
        with_tmp ~tmpdir ~expression:e "int-bin-comparison" (fun tmp ->
            let compute =
              rawf
                "{ if [ %s %s %s ] ; then printf true ; else printf false ; \
                 fi ; } > %s"
    ...
    | Getenv s ->
        let open Script in
        let string_script = continue s in
        with_tmp ~tmpdir ~expression:e ~script:string_script "getenv"
          (fun tmp ->
...
let compile ?(tmp_dir_path = `Fresh) ?(signal_name = "USR1")
  ...
  let tmp_files = ref [] in
  let module Comp = Make_compiler (struct
    let tmp_file_db = tmp_files
  end) in
  let open Comp.Script in
  let tmpdir =
  ...
  in
  let s = Comp.to_ir ~fail_commands ~tmpdir expr in
  let delete_tmps =
    rawf "%s () {\n%s\n}" delete_fname
    @@ String.concat ~sep:"\n"
         (List.map (List.dedup !tmp_files) ~f:(fun (tvariable, _) ->
              sprintf "  echo \"%s -> ${%s:-WUT}\" && rm -f \"${%s}\" || :"
                tvariable tvariable tvariable ))
  in
  let make_tmp_vars =
    List.concat_map (List.dedup !tmp_files) ~f:(fun (v, dir) ->
        [ cmtf "Making file %s" v
        ; Make_directory dir
        ; rawf "%s=%s/tmp-%s" v dir v ] )
  in
  ...
  let scr =
    make
      ...
      s.result
  in
  Script.Ready (scr, Comp.Script.pp)
